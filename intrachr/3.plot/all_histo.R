...
smc3 <- tf_zscore[,c("SMC3_r1","SMC3_r2")] %>% colMeans() %>% mean()
smc1 <- tf_zscore[,c("SMC1A_r1","SMC1A_r2")] %>% colMeans() %>% mean()
# wapl <- tf_zscore[,c("WAPL_r1","WAPL_r2")] %>% colMeans() %>% mean()
...
# p + geom_histogram(color = "black", fill = "gray") +
#   geom_vline(aes(xintercept = ctcf_mean), 
...
p +
  ...
  labs(title = "Mean Loop Z-Score Histogram",
       subtitle = "Human")+
  ylab('Dens.')+
  xlab('Mean of Z-Score')
ggsave(filename = "exless_mean_intrloop_zscore_hisogram.png",path = "../../figure/")  
#ggsave(filename = "new_intrloop_zscore_hisogram.pdf",path = "../../figure/")  
